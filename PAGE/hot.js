document.addEventListener("DOMContentLoaded", function() {
  ...
  if (genre) {
    ...
    const btn = Array.from(filterButtons)
      .find(b => b.textContent.trim().toLowerCase() === genre.toLowerCase());
    if (btn) {
  ...
  filterButtons.forEach(button => {
    button.addEventListener("click", () => {
      ...
      if (currentFilter === "Tất cả") {
        ...
        history.replaceState(null, '', '?genre=' + encodeURIComponent(currentFilter));
      }
    });
  });
  ...
  function displayStories(filteredList) {
    ...
    if (currentFilter !== "Tất Cả") {
      filteredStories = list.filter(story => story.category === currentFilter);
    }
